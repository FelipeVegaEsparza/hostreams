...
const PlanManagement = () => {
  ...
  const fetchPlans = async () => {
    ...
    try {
      ...
      const response = await axios.get(import.meta.env.VITE_API_BASE_URL + 'plans/all', config);
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditing) {
        await axios.put(import.meta.env.VITE_API_BASE_URL + `plans/${currentPlan.id}`, planData, config);
        toast.success('Plan actualizado.');
      } else {
        await axios.post(import.meta.env.VITE_API_BASE_URL + 'plans', planData, config);
