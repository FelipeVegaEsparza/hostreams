...
export const AuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    const loadUser = async () => {
      if (token) {
        try {
          console.log('Fetching user from URL:', import.meta.env.VITE_API_BASE_URL + 'auth/me');
          const response = await axios.get(import.meta.env.VITE_API_BASE_URL + 'auth/me'); // Obtener datos del usuario real
  ...
  const login = async (email, password) => {
    try {
      console.log('Login URL:', import.meta.env.VITE_API_BASE_URL + 'auth/login');
  ...
  const register = async (userData) => {
    try {
      console.log('Register URL:', import.meta.env.VITE_API_BASE_URL + 'auth/register');
